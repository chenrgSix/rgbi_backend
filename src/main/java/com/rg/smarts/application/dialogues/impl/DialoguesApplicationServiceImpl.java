...
public class DialoguesApplicationServiceImpl implements DialoguesApplicationService {
    ...
    public ChatVO chat(Long memoryId,String content, HttpServletRequest request) {
        ...
        String chatResponse = dialoguesDomainService.chat(content, loginUser.getId(),memoryId);
        ChatVO chatVO = new ChatVO();
        chatVO.setChatResponse(chatResponse);
        chatVO.setId(memoryId);
        return chatVO;
    }
    @Override
    public Long addDialoguesByUserId(String chatContent, Long userId) {
        return dialoguesDomainService.addDialoguesByUserId(chatContent,userId);
    }

    @Override
    public Boolean existDialoguesByUserId(Long memoryId, Long userId) {
        return dialoguesDomainService.existDialoguesByUserId(memoryId,userId);
    }
